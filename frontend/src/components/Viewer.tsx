...
export default function Viewer() {
  ...
  const { data: instancesData, isLoading: isLoadingInstances } = useQuery({
    ...
    queryFn: () => apiClient.queryInstances(
      pacsId, 
      studyInstanceUid || '', 
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      try {
        ...
        const imageIds = instancesData.instances.map((instance: Instance) => 
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900">
                ...
                {seriesData.series.map((series: Series) => (
                  ...
                  >
                    ...
                    <div className="text-gray-400 mt-1">
                      {series.Modality || 'Unknown'} • {series.NumberOfSeriesRelatedInstances || '?'} images
                    </div>
                  </button>
                ))}
              </div>
      ...
      <div className="p-2 text-white text-sm bg-gray-800 border-t border-gray-700 flex justify-between">
        <span>
          Study: {studyInstanceUid?.slice(0, 30)}...
        </span>
        <span>
          {viewerState.selectedSeriesUid ? (
